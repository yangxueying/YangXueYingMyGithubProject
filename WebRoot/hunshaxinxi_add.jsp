...
function hsgxia2shxurxu(nstr,nwbk)
{
	...
	<body>
			<form action="addHunshaxinxi.do" name="form1" method="post">
				      <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1"  style="border-collapse:collapse" bgcolor="#F2FDFF">
						<tr bgcolor="#E7E7E7">
							<td height="30" colspan="2" background="images/table_header.gif">添加婚纱信息</td>
						</tr>
...
<script language=javascript >  
function hsgcheck() {
		var hunshamingcheng = $("#hunshamingcheng").val();
		if(hunshamingcheng==""||(hunshamingcheng.length<3||hunshamingcheng.length>12)){
			 $("#clabelhunshamingcheng").html("<font color=red>婚纱名称不能为空且长度在3～12位之间！</font>");
			 $("input[id=hunshamingcheng]").focus();
			 $("#querenzhuce").attr("disabled","disabled");
			 return false;
		} 
		else
		{
			$("#clabelhunshamingcheng").html("");
			$.ajax({
				url : "quchongHunshaxinxi.do",
				type : "post",
				data : "hunshamingcheng=" + hunshamingcheng,
				dataType : "json",
				success:function(result){
				if(result.info=="ng"){
					$("#clabelhunshamingcheng").html("<font color=red>婚纱名称已存在，请更换！</font>");
					$("input[id=hunshamingcheng]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
				...
				error:function(){
					$("#clabelhunshamingcheng").html("系统异常，请检查错误！");
					$("input[id=hunshamingcheng]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
